...
function* registerSaga({
  payload: { userReq, emailQuery, pixelIdQuery, onSuccess, onFailure },
}) {
  try {
    ...
    let pixelSlug = null;
    // if (emailQuery && pixelIdQuery) {
    //   const { data, errors } = yield call(
    //     pixelService.joinPixel,
    //     emailQuery,
    //     pixelIdQuery
    //   );
    //   if (errors) {
    ...
    if (session && user) yield fork(setUserLocalSaga, user);
    if (_.isFunction(onSuccess)) onSuccess(pixelSlug);
  } catch (e) {
...
function* signInSaga({
  payload: { email, password, emailQuery, pixelIdQuery, onSuccess, onFailure },
}) {
  try {
...
function* searchEmailOrNameSaga({
  payload: { userId, pixelId, searchText, onSuccess, onFailure },
}) {
  try {
    const { data, errors } = yield call(
      authService.search,
